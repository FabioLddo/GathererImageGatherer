...
def gemini_captioner(image_path, api_key):
    ...
    # TODO: add context about images(magic, fantasy ecc.)
...
def save_processed_images(processed):
...
def main():
    ...
    for set_name in sets:
        if set_name not in processed:
            processed[set_name] = []
    
        for image_name in os.listdir(os.path.join("data/images/", set_name)):
            if not any(image_name.lower().endswith(ext) for ext in ('.png', '.jpg', '.jpeg', '.bmp')):
                continue

            if image_name in processed[set_name]:
                # Already processed
                continue

            image_path = os.path.join("data/images/", set_name, image_name)
            ...
            if response:
                ...
                processed[set_name].append(image_name)
                save_processed_images(processed)
            else:
                print(f"Failed to generate caption for {image_name} after {MAX_RETRIES} attempts.")
